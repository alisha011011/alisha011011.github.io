...
try:
    client = client.Client('wss://mycosmos-account.documents.azure.com:443/', 'g',
                           username="/dbs/mydatabase/colls/mygraph",
                           password="YRdBKq745edIgkX3Nz6hR33vDSOrCWQMyl0ZdXVtcaTBdxAzGaIIswxGkTTEKzRvTB2xtBi2meRaxXSNu3vegQ=="
    ...
    print("We're about to drop whatever graph is on the server. Press any key to continue...")
    #cleanup_graph(client)

    # Insert all vertices
    print("Let's insert some vertices into the graph. Press any key to continue...")
    insert_vertices(client)

    # Create edges between vertices
    print("Now, let's add some edges between the vertices. Press any key to continue...")
    insert_edges(client)

    # Update a couple of vertices
    print("Ah, sorry. I made a mistake. Let's change the ages of these two vertices. Press any key to continue...")
    update_vertices(client)

    # Count all vertices
    print("Okay. Let's count how many vertices we have. Press any key to continue...")
    count_vertices(client)

    # Execute traversals and get results
    print("Cool! Let's run some traversals on our graph. Press any key to continue...")
    execute_traversals(client)

    # Drop a few vertices and edges
    print("So, life happens and now we will make some changes to the graph. Press any key to continue...")
    execute_drop_operations(client)

    # Count all vertices again
    print("How many vertices do we have left? Press any key to continue...")
    count_vertices(client)
